...
CREATE TABLE `tbl_lugares` (
  ...
  `numero_lugar` varchar(2) DEFAULT NULL,
  `estado` enum('livre','ocupado') DEFAULT NULL,
  PRIMARY KEY (`lugar_id`),
  KEY `user_fk` (`user_fk`),
  KEY `voo_fk` (`voo_fk`),
  CONSTRAINT `tbl_lugares_ibfk_1` FOREIGN KEY (`user_fk`) REFERENCES `tbl_users` (`user_id`),
  CONSTRAINT `tbl_lugares_ibfk_2` FOREIGN KEY (`voo_fk`) REFERENCES `tbl_voos` (`voo_id`)
